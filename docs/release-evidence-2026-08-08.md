...
- Command: `.\gradlew.bat test ktlintCheck buildPlugin verifyPlugin -PccbBuildDir=build-final-release21 --no-daemon --console=plain`.
- Result: `BUILD SUCCESSFUL`; 220 tests, 0 failures, 0 errors and 0 skipped tests.
- The release21 gate passed all 220 tests, including external restore, runtime resolver handlers, non-Python whole-file patch operations, relation identity, prompt ordering/import migration, previous-batch filtering, the Chosen/Pinned context-files view, responsive Batch layout and ignored-tree guideline discovery.
...
- Current release21 ZIP produced by `buildPlugin`: 846678 bytes, SHA-256 `F643480A464ED318BDE5592BFBE3C52806B34F93A9A1878322ECDC8812C00793`.
- Final verification ZIP: `build-final-release21/distributions/copilot-context-bridge-1.0.0.zip`.
- Final verification size: 846678 bytes.
- Final verification SHA-256: `F643480A464ED318BDE5592BFBE3C52806B34F93A9A1878322ECDC8812C00793`.
...
- `install.ps1 -BuildOnly` was run successfully after release21; it produced a fresh ZIP and copied it to `build/distributions`.
...
- A full `install.ps1` run after release21 completed successfully after closing the controlled test instance and atomically installed `%APPDATA%/JetBrains/PyCharm2026.2/plugins/copilot-context-bridge`.
