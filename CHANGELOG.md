...
- Extended patch import with complete add/replace/delete-file operations alongside function replacement/addition.
...
- Replaced the unstable More tab strip with a fixed card workspace so destinations never jump, wrap or reorder.
- Made staging asynchronous and transactional; failed preparation removes partial sessions and never blocks retry.
- Added UTF-8 BOM and Windows CRLF handling for both strict `changes.json` ZIPs and source-only fallback ZIPs.
...
- Replaced the two narrow Batch file columns with one full-width Pinned/Automatic dropdown and scrollable exact file list.
- Added an editable, validated kickoff prompt directly below the prepared drag zone and included it in complete text copies while retaining the full More → Context preview.
- Added resolver/policy provenance, character/token-aware deterministic bundles, fuzzy matching tests and richer per-category packing summaries.
- Added project secret patterns and scoped exclusions for external/archive context sources.
- Made structured import select every proposed change by default while retaining conflict blocking, source-only ZIP opt-in review, concrete blockers, delete warnings and in-memory native merge proposals.
